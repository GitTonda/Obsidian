Stabilire una visione comune e la portata del progetto
- 10% dei [[Requisiti & Casi d'uso]]
- requisiti non funzionali critici
- studio economico
- preparazione ambiente di sviluppo
- periodo di breve durata

L'obbiettivo NON è di definire tutti i requisiti, bensì si decide se il progetto merita un indagine più seria durate l'elaborazione

#### Elaborati:
- Visione e Studio economico
- Modello dei [[Requisiti & Casi d'uso]]
- Specifiche supplementari
- Glossario
- Lista dei Rischi e di Gestione dei Rischi
- Prototipi e proof of concept
- Piano dell'iterazione
- Piano delle fasi di Sviluppo
- Scenario di sviluppo
è una quantità eccessiva di documentazione, ma deve essere preliminare ed approssimativa.

